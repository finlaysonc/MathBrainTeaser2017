        ...
        foreach (string pattern in patterns)
        {
            ...
            foreach (IList<int> dig in dPerms)
            {
                foreach (IList<int> opr in oPerms)
                {
                    ...
                    int result = (int) _processor.Solve<NumberResult>(candidate).Result;
//                    int result = _processor.Solve<int>(candidate);
                    if (result == goal)
                    {
                        solution = _processor.Parse(candidate).ToString();
                        return true;
                    }
                    //NCalc.Expression e = new NCalc.Expression(candidate);
                    //if ((int) e.Evaluate() == goal)
                    //{
                    //    solution = e.ParsedExpression.invisual
                    //}
